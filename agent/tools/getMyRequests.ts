 ...
 * tool directly, so it must stay `getMyRequests`.
 *
...
export default defineTool({
  description: SUPPORT_SPECS.getMyRequests.description,
  inputSchema: SUPPORT_SPECS.getMyRequests.inputSchema,
  execute: async (input, ctx) => {
    const runtime = await supportRuntime(ctx)
    if (!runtime) return NOT_A_SUPPORT_TURN

    return createSupportTools(runtime.context).getMyRequests.execute!(input, execOptions(ctx.callId))
  },
})
