 ...
 * tool directly, so it must stay `addGlossaryEntry`.
 *
...
export default defineTool({
  description: SUPPORT_SPECS.addGlossaryEntry.description,
  inputSchema: SUPPORT_SPECS.addGlossaryEntry.inputSchema,
  execute: async (input, ctx) => {
    const runtime = await supportRuntime(ctx)
    if (!runtime) return NOT_A_SUPPORT_TURN

    return createSupportTools(runtime.context).addGlossaryEntry.execute!(input, execOptions(ctx.callId))
  },
})
