 ...
 * tool directly, so it must stay `fileRequest`.
 *
...
export default defineTool({
  description: SUPPORT_SPECS.fileRequest.description,
  inputSchema: SUPPORT_SPECS.fileRequest.inputSchema,
  execute: async (input, ctx) => {
    const runtime = await supportRuntime(ctx)
    if (!runtime) return NOT_A_SUPPORT_TURN

    return createSupportTools(runtime.context).fileRequest.execute!(input, execOptions(ctx.callId))
  },
})
